...
namespace block_painelaulas\local;
...
class source {
    ...
    public static function enrol_session(int $userid, int $sessionid): array {
        ...
        if (!self::is_fallback_enabled()) {
            throw new moodle_exception('integrationmissing', 'block_painelaulas');
        }

        $session = self::find_fallback_session($sessionid);
        if (!$session) {
            throw new moodle_exception('error:sessionnotfound', 'block_painelaulas');
        }
        ...
        return [
            'status' => true,
            'message' => get_string('enrolsuccess', 'block_painelaulas'),
            'usingfallback' => true,
        ];
    }
    ...
    protected static function call_provider(string $callback, array $params, array $fallback): array {
        ...
        if (!self::is_fallback_enabled()) {
            throw new moodle_exception('error:missingprovider', 'block_painelaulas');
        }
    ...
    protected static function normalise_session(array $session): array {
        ...
        if (!$imageurl) {
            $imageurl = (new moodle_url('/blocks/painelaulas/pix/banner-placeholder.svg'))->out(false);
        }
    ...
    protected static function is_fallback_enabled(): bool {
        return (bool)get_config('block_painelaulas', 'enablefallback');
    }
    ...
    protected static function get_fallback_sessions(): array {
        $now = time();
        $day = 86400;
        $image = (new moodle_url('/blocks/painelaulas/pix/banner-placeholder.svg'))->out(false);
