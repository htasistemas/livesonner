...
define(['core/ajax', 'core/notification'], function(Ajax, Notification) {
    const SELECTORS = {
        panel: type => `.block-painelaulas__panel[data-panel="${type}"]`,
    ...
    const renderPanel = (type, sessions) => {
        ...
        if (!sorted.length) {
            ...
            empty.className = 'block-painelaulas__empty';
    ...
    const createCardEntry = (session, panelType) => {
        ...
        article.className = 'block-painelaulas__card';
        article.dataset.panel = panelType;
        article.dataset.sessionId = session.id;

        const imageWrapper = document.createElement('div');
        imageWrapper.className = 'block-painelaulas__card-image';
        ...
        overlay.className = 'block-painelaulas__card-overlay';
        imageWrapper.appendChild(overlay);

        const countdown = document.createElement('div');
        countdown.className = 'block-painelaulas__card-countdown';
        imageWrapper.appendChild(countdown);

        const info = document.createElement('div');
        info.className = 'block-painelaulas__card-info';

        const title = document.createElement('h3');
        title.className = 'block-painelaulas__card-title';
        title.textContent = session.name;
        info.appendChild(title);

        const meta = document.createElement('div');
        meta.className = 'block-painelaulas__card-meta';
        ...
        if (Array.isArray(session.tags) && session.tags.length) {
            ...
            tagsContainer.className = 'block-painelaulas__card-meta';
            session.tags.forEach(tag => {
                const chip = document.createElement('span');
                chip.className = 'block-painelaulas__chip';
        ...
        if (session.summary) {
            ...
            description.className = 'block-painelaulas__card-description';
            description.innerHTML = session.summary;
            info.appendChild(description);
        }

        const footer = document.createElement('div');
        footer.className = 'block-painelaulas__card-footer';

        if (session.isenrolled && panelType === 'catalog') {
            const badge = document.createElement('span');
            badge.className = 'block-painelaulas__badge';
        ...
        button.type = 'button';
        button.className = 'block-painelaulas__button';
    ...
    const appendMeta = (container, label, value) => {
        ...
        item.className = 'block-painelaulas__card-meta-item';
    ...
    const createAgendaItem = session => {
        ...
        item.className = 'block-painelaulas__agenda-item';

        const time = document.createElement('div');
        time.className = 'block-painelaulas__agenda-time';
        time.textContent = buildAgendaTime(session);

        const name = document.createElement('div');
        name.className = 'block-painelaulas__agenda-name';
        name.textContent = session.name;

        const status = document.createElement('div');
        status.className = 'block-painelaulas__agenda-status';
    ...
    const updateCountdown = entry => {
        ...
        if (!start || now < start) {
            ...
            entry.countdown.innerHTML = `<span class="block-painelaulas__card-countdown-label">${state.config.strings.countdownlabel}</span>${formatDuration(diff)}`;
        } else if (now >= start && now <= end) {
            state = 'live';
            stateLabel = state.config.strings.agendalive;
            const remaining = Math.max(0, end - now);
            entry.countdown.innerHTML = `<span class="block-painelaulas__card-countdown-label">${state.config.strings.countdownlive}</span>${formatDuration(remaining)}`;
        } else {
            state = 'past';
            stateLabel = state.config.strings.agendapast;
            entry.countdown.innerHTML = `<span class="block-painelaulas__card-countdown-label">${state.config.strings.countdownfinished}</span>${buildDate(start)}`;
        }
        ...
        if (entry.agendaItem) {
            ...
            const status = entry.agendaItem.querySelector('.block-painelaulas__agenda-status');
            if (status) {
